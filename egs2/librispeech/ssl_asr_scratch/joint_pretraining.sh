...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.ssl_asr_scratch_train \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/text_collapsed,text_collapsed,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text_collapsed,text_collapsed,text" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text_collapsed,text_collapsed,text "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text_collapsed,text_collapsed,text "
