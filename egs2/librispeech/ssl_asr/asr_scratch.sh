...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.ssl_asr_scratch_train \
                --collect_stats true \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/text,text,text_int" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/text_collapsed,text_collapsed,text_int" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text_int" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text_collapsed,text_collapsed,text_int" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
        ...
        <"${asr_stats_dir}/train/text_shape" \
            awk -v N=501 '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_shape.${token_type}"

        <"${asr_stats_dir}/train/text_collapsed_shape" \
            awk -v N=501 '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_collapsed_shape.${token_type}"

        <"${asr_stats_dir}/valid/text_shape" \
            awk -v N=501 '{ print $0 "," N }' \
            >"${asr_stats_dir}/valid/text_shape.${token_type}"

        <"${asr_stats_dir}/valid/text_collapsed_shape" \
            awk -v N=501 '{ print $0 "," N }' \
            >"${asr_stats_dir}/valid/text_collapsed_shape.${token_type}"
    fi
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text,text,text_int "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text_collapsed,text_collapsed,text_int "
            _opts+="--train_shape_file ${_split_dir}/speech_shape "
            _opts+="--train_shape_file ${_split_dir}/text_shape.${token_type} "
            _opts+="--train_shape_file ${_split_dir}/text_collapsed_shape.${token_type} "
            _opts+="--multiple_iterator true "

        else
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${_scp},speech,${_type} "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text,text,text_int "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text_collapsed,text_collapsed,text_int "
            _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_shape.${token_type} "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_collapsed_shape.${token_type} "
        fi
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.ssl_asr_scratch_train \
                ...
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/wav.scp,speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text_int" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text_collapsed,text_collapsed,text_int" \
                --valid_shape_file "${asr_stats_dir}/valid/speech_shape" \
                --valid_shape_file "${asr_stats_dir}/valid/text_shape.${token_type}" \
                --valid_shape_file "${asr_stats_dir}/valid/text_collapsed_shape.${token_type}" \
                --resume true \
