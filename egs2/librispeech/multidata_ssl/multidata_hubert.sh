...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    # Manual data preparation. wav.scp and similar files of datasets have to be combined
    ...
    for ((iter=${pretrain_start_iter}; iter<=${pretrain_stop_iter};iter++)); do
        ...
        if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
            ...
            ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
                         ${python} -m espnet2.bin.multidata_hubert_train \
                         --collect_stats true \
                         ...
                         --train_data_path_and_name_and_type "${_asr_train_dir}/dataset_type,dataset_type,int" \
                         --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                         --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                         --valid_data_path_and_name_and_type "${_asr_valid_dir}/dataset_type,dataset_type,int" \
        ...
        if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
            ...
            if [ "${num_splits_asr}" -gt 1 ]; then
                ...
                _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/dataset_type,dataset_type,int "
