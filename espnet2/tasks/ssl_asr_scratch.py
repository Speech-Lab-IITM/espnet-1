...
from espnet2.ssl_asr_scratch.espnet_model import ESPnetSSLASRScratchModel
...
class SSL_ASRFromScratchTask(AbsTask):
    ...
    def add_task_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            "--model_conf",
            action=NestedDictAction,
            default=get_default_kwargs(ESPnetSSLASRScratchModel),
    ...
    def required_data_names(
        ...
        if not inference:
            retval = ("speech", "text", "text_collapsed",)
        else:
            # Recognition mode
            retval = ("speech",)
        return retval
    ...
    @classmethod
    def build_model(cls, args: argparse.Namespace) -> ESPnetSSLASRScratchModel:
        assert check_argument_types()        
        ...
        # 8. Build model
        model = ESPnetSSLASRScratchModel(
            vocab_size=vocab_size,
            frontend=frontend,
            specaug=specaug,
            normalize=normalize,
