...
# Created by Arun Kumar A(CS20S013), Speech Lab IIT Madras
class MultilingualEDDASRModel(AbsESPnetModel):
    """CTC-attention hybrid Encoder-Decoder-Decoder multilingual model"""
    ...
    def __init__(
        ...
        if self.use_transducer_decoder:
            ...
            if ctc_weight == 1.0:
                logging.warning('ctc only training is not supported in this setup')
                exit(1)
            else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert text2_lengths.dim() == 1, text2_lengths.shape
        ...
        assert (
            ...
            == text2.shape[0]
            == text2_lengths.shape[0]
        ), (speech.shape, speech_lengths.shape, text.shape, text_lengths.shape, text2.shape, text2_lengths.shape)
        batch_size = speech.shape[0]

        # for data-parallel
        text = text[:, : text_lengths.max()]
        text2 = text2[:, : text2_lengths.max()]
