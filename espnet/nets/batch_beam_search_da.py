...
class BatchBeamSearchDA(BeamSearchDA):
    """Batch beam search implementation for dual attention decoder."""
    ...
    def score_full(
        self, hyp: BatchHypothesis, x: torch.Tensor, x2: torch.Tensor
        ...
        for k, d in self.full_scorers.items():
            if isinstance(d, BatchScorerInterfaceDA):
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], x, x2)
            else:
    ...
    def search(self, running_hyps: BatchHypothesis, x: torch.Tensor, x2: torch.Tensor=None) -> BatchHypothesis:
        ...
        scores, states = self.score_full(running_hyps, x.expand(n_batch, *x.shape), x2.expand(n_batch, *x2.shape) if x2 is not None else None)
